...
class MockNemotronProvider(AIProvider):
    ...
    async def solve_math(self, base64_image: Optional[str] = None, equation_text: Optional[str] = None) -> Dict[str, Any]:
        ...
        if not target_text and base64_image:
            target_text = extract_text_from_base64_image(base64_image).strip()

        if target_text:
            from app.services.ai.math_engine import math_engine
            return math_engine.solve(target_text)
        ...
        return {
            ...
            "steps": [
                ...
                "3. Ensure the text is well-lit and not blurry, or enter the equation directly."
            ],
            "final_answer": "N/A",
            "explanation": "Please capture a clear, well-lit photo of the equation or type the problem directly."
        }
...
class OpenRouterNemotronProvider(AIProvider):
    ...
    async def solve_math(self, base64_image: Optional[str] = None, equation_text: Optional[str] = None) -> Dict[str, Any]:
        ...
        2. If base64_image is provided, uses multimodal vision models (or OCR) to inspect and solve the problem.
        3. Returns structured JSON containing step-by-step logic, concepts, and answers.
        """
        clean_text = (equation_text or "").strip()
        if not clean_text and base64_image:
        ...
        # Case 1: We have text (either supplied or OCR'd)
        if clean_text:
            prompt = (
                "You are an expert mathematician and highly capable AI tutor.\n"
                f"A student has asked to solve the following math problem:\n\n{clean_text}\n\n"
        ...
        if base64_image:
            image_prompt = (
                "You are an expert mathematician and highly capable AI tutor.\n"
                "A student has taken a photo of a math problem with their mobile camera.\n"
                "Read the math problem in the image carefully, solve it, and provide a structured JSON response:\n"
                "1. 'problem': Transcribe the exact problem or equation from the image.\n"
                "2. 'category': Math branch (e.g., Algebra, Calculus, Geometry, Arithmetic).\n"
                "3. 'difficulty': Level (Beginner, Intermediate, Advanced).\n"
                "4. 'key_concepts': 1 to 3 core concepts/rules used.\n"
                "5. 'steps': Step-by-step breakdown as a list of strings.\n"
                "6. 'final_answer': Direct final answer (e.g. 'x = 12').\n"
                "7. 'explanation': Clear explanation of why this solution is correct.\n\n"
                "Output strictly as a JSON object with these keys. Do not include markdown code ticks."
            )
            vision_models = [
                "nvidia/nemotron-3-nano-omni-30b-a3b-reasoning:free",
